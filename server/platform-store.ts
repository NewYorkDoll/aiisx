...
async function ensureSchema() {
  ...
  if (!schemaReady) {
    schemaReady = Promise.all([
      ...
      pool.query('CREATE TABLE IF NOT EXISTS xbox_game_activity (title_id VARCHAR(128) PRIMARY KEY, name VARCHAR(255) NOT NULL, played_at DATETIME NULL, cover VARCHAR(500) NULL, gamerscore INT NOT NULL, achievements INT NOT NULL, synced_at DATETIME NOT NULL)'),
...
export async function saveXboxSnapshot(snapshot: XboxSnapshot) {
  ...
  for (const game of snapshot.games) await pool.execute('INSERT INTO xbox_game_activity (title_id, name, played_at, cover, gamerscore, achievements, synced_at) VALUES (?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE name=VALUES(name), played_at=VALUES(played_at), cover=VALUES(cover), gamerscore=VALUES(gamerscore), achievements=VALUES(achievements), synced_at=VALUES(synced_at)', [game.titleId, game.name, asDate(game.playedAt), game.cover, game.gamerscore, game.achievements, asDate(snapshot.fetchedAt)])
}
...
export async function getStoredXboxSnapshot() {
  ...
  if (!profile) return null
  const [games] = await pool.query('SELECT title_id, name, played_at, cover, gamerscore, achievements FROM xbox_game_activity ORDER BY played_at DESC, synced_at DESC LIMIT 5')
  return { configured: true, profile: { xuid: profile.xuid, gamertag: profile.gamertag, displayName: profile.display_name, avatar: profile.avatar, gamerscore: profile.gamerscore }, state: profile.state, currentGame: profile.current_game, games: (games as Array<{ title_id: string; name: string; played_at: Date | null; cover: string | null; gamerscore: number; achievements: number }>).map((game) => ({ titleId: game.title_id, name: game.name, playedAt: game.played_at ? asIso(game.played_at) : null, cover: game.cover, gamerscore: game.gamerscore, achievements: game.achievements })), fetchedAt: asIso(profile.fetched_at) } satisfies XboxSnapshot
}
...
export async function getStoredFitnessSnapshot() {
  ...
  return row ? { weight: row.weight === null ? null : Number(row.weight), weightUnit: row.weight_unit, sessions: row.sessions, minutes: row.minutes, planName: row.plan_name, todayName: row.today_name, fetchedAt: asIso(row.fetched_at) } satisfies FitnessSnapshot : null
}
