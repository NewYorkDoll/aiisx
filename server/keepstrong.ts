...
type RecordItem = { id?: string; status?: string; durationSeconds?: number; name?: string; dayStr?: string; createdAt?: string; actions?: Array<{ name?: string; groups?: TrainingGroup[] }> }
...
export async function fetchFitnessSnapshot(): Promise<FitnessSnapshot> {
  ...
  const recentSets: FitnessSetSummary[] = completed
    .slice()
    .sort((a, b) => (b.dayStr || b.createdAt || '').localeCompare(a.dayStr || a.createdAt || ''))
    .flatMap((record) => {
      let sequence = 0
      return (record.actions || []).flatMap((action) => (action.groups || []).map((group, groupIndex) => ({
        id: group._id || `${record.id || record.dayStr || 'session'}-${sequence}`,
        planName: record.name || 'workout',
        actionName: action.name || 'exercise',
        date: record.dayStr || record.createdAt || '',
        reps: String(group.reps ?? ''),
        setNumber: groupIndex + 1,
        sequence: sequence++,
      }))).reverse()
    })
    .slice(0, 10)
